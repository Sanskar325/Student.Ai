...
const navItems = [
    ...
    {to: '/ai/aicalculator', label: 'AiCalculator', Icon: SquarePen},
      ...
      <div className='my-7 w-full'>
        ...
        <div className='px-6 mt-5 text-sm text-gray-600 font-medium'>
            {navItems.map(({to, label, Icon})=>(
                <NavLink key={to} to={to} end={to === '/ai'} onClick={()=> setSidebar(false)} className={({isActive})=> `px-3.5 py-2.5 flex items-center gap-3 rounded ${isActive ? 'bg-gradient-to-r from-[#722344] to-[#A0394C] text-white' : ''}`}>
                    {({ isActive })=>(
                        <>
            ...
            <div onClick={openUserProfile} className='flex gap-2 items-center cursor-pointer'>
                ...
                <div>
                    ...
                    <p className='text-xs text-gray-500'>
                        <Protect plan='premium' fallback="Free"></Protect> Plan
                    </p>
                </div>
            </div>
