    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
          <div className='flex items-center gap-3'>
            <Sparkles className='w-6 text-[#9D2235]'/>
          ...
          <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
            {imageStyle.map((item)=>(
              ...
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedStyle === item ? 'bg-green-50 text-[#722F37]' : 'text-gray-500 border-gray-300'}`} key={item}>{item}</span>
            ) )}
          </div>
            ...
            <div className='my-6 flex items-center gap-2'>
              <label className='relative cursor-pointer'>
                ...
                <div className='w-9 h-5 bg-slate-300 rounded-full peer-checked:bg-[#722F37] transition'></div>
          ...
          <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#9D2235] to-[#722F37] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'>
            {
